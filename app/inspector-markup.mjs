...
export function createInspectorMarkup({ state, t, referenceRightsMarkup }) {
  ...
  function detailTagsSectionMarkup(asset) {
    ...
    return `<section class="inspector-section detail-tags-section" data-inspector-section="tags" aria-label="${escapeHtml(t("tags"))}"><div class="detail-tags-row" data-tags-list>${sourceMarkup}${tagMarkup}<button class="detail-tags-add" type="button" data-action="add-tag"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M12 5v14M5 12h14"/></svg><span>${escapeHtml(t("addTag"))}</span></button></div><div class="asset-curation-region" data-curation-region>${curationMarkup(asset)}</div></section>`;
  }

  function curationMarkup(asset) {
    const curated = asset.curated === true;
    const note = String(asset.curation_note || "");
    return `<div class="asset-curation-head"><div><strong>${escapeHtml(t("curationTitle"))}</strong><span>${escapeHtml(t("curationDescription"))}</span></div><button class="action-btn secondary asset-curation-toggle${curated ? " active" : ""}" type="button" data-action="toggle-curated" aria-pressed="${curated}">${escapeHtml(curated ? t("curatedSelected") : t("markCurated"))}</button></div><label class="asset-curation-note"><span>${escapeHtml(t("curationNote"))}</span><textarea data-curation-note maxlength="4000" rows="4" placeholder="${escapeHtml(t("curationNotePlaceholder"))}">${escapeHtml(note)}</textarea></label><div class="asset-curation-actions"><button class="action-btn secondary" type="button" data-action="save-curation">${escapeHtml(t("saveCuration"))}</button></div><div class="context-package-actions"><div><strong>${escapeHtml(t("reuseContext"))}</strong><span>${escapeHtml(t("contextPackageDescription"))}</span></div><div><button class="action-btn secondary" type="button" data-action="copy-context-package">${escapeHtml(t("copyContextPackage"))}</button><button class="action-btn secondary" type="button" data-action="export-context-package">${escapeHtml(t("exportContextPackage"))}</button></div></div>`;
  }
  ...
  return { fileDimensionsText, fileFormatText, fileSizeText, fileAspectRatioText, formatFileSize, fileFactRowMarkup, fileFactTagMarkup, detailFavoriteButtonMarkup, detailFileSectionMarkup, detailPromptSectionMarkup, promptReferencesMarkup, editRecipeFieldsMarkup, detailSourceSectionMarkup, detailVersionSectionMarkup, versionPickerMarkup, versionCompareMarkup, versionOptionLabel, detailVersionSummaryMarkup, detailGroupSectionMarkup, detailTagsSectionMarkup, curationMarkup, detailMoreSectionMarkup, versionHistoryMarkup, generationHistoryMarkup, recipeHistoryDisclosureMarkup, referenceRightsSummary, recipeHistoryMarkup, categoryOptions, buildSourceRows, sourceName, sourceCopyValue, isVideoAsset, assetMediaPreviewMarkup, stackInspectorMarkup };
}
