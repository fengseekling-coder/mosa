...
The Windows target keeps the shared renderer and runtime code, uses Windows-native `better-sqlite3` and Sharp binaries, and hides Electron's native application menu bar while retaining menu accelerators. Development packages may remain unsigned, while `desktop:release:windows` is the signed portable-release path and fails closed without an explicit Authenticode signing source plus pinned signer thumbprint. In-app updates verify the signed release manifest, artifact size/SHA-256, every executable/native payload signature, and the relaunched build identity before committing replacement. Grok and Cowart source discovery on Windows and a conventional installer remain separate release work.
...
> **Project status:** macOS has a release-grade Developer ID/notarized DMG path through `npm run desktop:release`; local development DMGs remain ad-hoc and are never accepted by the release gate. Windows remains a preview target, but release portable ZIPs now have an explicit Authenticode path through `npm run desktop:release:windows`. Both release paths require immutable remote source provenance and a signed release manifest; a conventional Windows installer is still future work.
