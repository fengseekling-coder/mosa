...
Desktop builds enable minimal anonymous usage metrics by default so the project can measure whether releases are actually being installed and retained. The setting can be turned off at any time in Preferences. Turning it off does not disable update checks.

When enabled, MOSA stores a random UUID in Electron's local `userData` directory. The UUID is generated by MOSA and is not derived from a device serial number, account, filesystem path, hardware fingerprint, Prompt, asset, or source-tool identifier.

At most once in a 24-hour period, MOSA may attach the following bounded fields to the existing request for `https://mosa.azhuilab.com/releases/latest.json`:
