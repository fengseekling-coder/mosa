...
- **Masonry and navigation containment:** Gallery cards use their measured outer height with a stable ~8px vertical gutter, so no card background is exposed beneath an image and adjacent items do not develop visibly uneven gaps. Sidebar collection labels remain within the navigation width, use an ellipsis when needed, and expose the complete name through the native hover tooltip.
