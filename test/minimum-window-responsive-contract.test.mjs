...
test("10-11. 退役 Finder IPC 保持移除且其余 Desktop IPC 不变", async () => {
  ...
  for (const channel of ["paste-image", "set-locale"]) {
    assert.match(main, new RegExp(`ipcMain\\.handle\\("${channel}"`));
  }
  // 更新检查、下载安装都不接受 renderer 提供的 URL；其余能力仍保持封闭。
  assert.equal(preload.split("ipcRenderer.invoke").length - 1, 16, "no invoke channel beyond the currently approved narrow requests, including the Visual Pack lifecycle");
});
...
test("40-41. package 与 lockfile 不变、无新依赖", async () => {
  ...
  assert.equal(sha256(lock), "b3f46d8c3c4734f3ae35fd0eb2f8a5202ed56b399060037239bb36f67dae6d02");
});
