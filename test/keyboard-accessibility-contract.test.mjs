...
const checks = [
  ["01 no positive literal tabindex", async () => {
    const [html, app] = await Promise.all([read("app/index.html"), read("app/app.js")]);
  ...
  ["03 one application shortcut router", async () => {
    ...
    const galleryRouter = sliceBetween(app, "function bindKeyboardNav(", "// ===== Phase 5C");
    assert.doesNotMatch(galleryRouter, /document\.addEventListener\("keydown"/);
  }],
  ["04 confirm blocks background shortcuts", async () => assert.match(await read("app/app.js"), /if \(confirmDialogState\.pending\) return;/)],
  ...
  ["08 viewer Escape returns to library", async () => assert.match(await read("app/app.js"), /if \(state\.viewMode === "asset"\) \{ returnToLibrary\(\); event\.preventDefault\(\); return; \}/)],
  ["09 form controls guard global shortcuts", async () => assert.match(await read("app/app.js"), /if \(event\.target\.matches\("input, textarea, select"\)\)/)],
  ...
  ["11 modified viewer shortcuts are native", async () => {
    const slice = functionSlice(await read("app/app.js"), "// Phase 3B / 规格 §8：专用大图舞台缩放快捷键", 'if (event.key === "b"');
  ...
  ["13 filter opens on facet search", async () => assert.match(await read("app/app.js"), /focusOnOpen: \(\) => els\.facetSearchInput/)],
  ["14 settings roving tabindex remains", async () => assert.match(await read("app/app.js"), /function primeSettingsRoving\(\)[\s\S]*?item\.tabIndex = index === 0 \? 0 : -1;/)],
  ["15 language roving remains", async () => assert.match(await read("app/app.js"), /function focusLanguageMenuItem\(item\)[\s\S]*?item\.tabIndex = 0;/)],
  ...
  ["17 gallery card has native button entry", async () => assert.match(await read("app/app.js"), /<button class="asset-card-select" type="button"/)],
  ["18 keyboard card activation uses openAssetView", async () => assert.match(await read("app/app.js"), /openAssetView\(id, button\)/)],
  ["19 viewer entry focuses Return", async () => assert.match(await read("app/app.js"), /els\.assetViewBack\?\.focus\(\);/)],
  ["20 hidden library is inert in viewer", async () => assert.match(await read("app/app.js"), /els\.libraryView\.toggleAttribute\("inert", assetMode\)/)],
  ...
  ["24 copy prompt action is native", async () => assert.match(await read("app/app.js"), /data-action="copy-prompt"/)],
  ["25 copy source action is native", async () => assert.match(await read("app/app.js"), /data-action="copy-source"/)],
  ["26 copy path action is native", async () => assert.match(await read("app/app.js"), /data-action="copy-path"/)],
  ["27 Cowart target is reachable", async () => assert.match(await read("app/app.js"), /data-cowart-insert-target/)],
  ["28 Cowart insert is reachable", async () => assert.match(await read("app/app.js"), /data-action="insert-cowart"/)],
  ["29 More disclosure is native", async () => assert.match(await read("app/app.js"), /data-more-actions/)],
  ["30 App/Web original media split remains", async () => {
    const app = await read("app/app.js");
    assert.match(app, /data-action="show-in-finder"/);
    assert.match(app, /class="action-btn secondary original-media-link"/);
  ...
  ["33 discrete viewer zoom announces percent", async () => assert.match(await read("app/app.js"), /function announceAssetViewZoom\(\)[\s\S]*?zoomAnnouncement/s)],
  ["34 unchanged viewer zoom does not announce", async () => assert.match(await read("app/app.js"), /Math\.abs\(scale - assetViewTransform\.scale\) <= ASSET_VIEW_SCALE_EPSILON[\s\S]*?return false;/s)],
  ...
  ["38 preview reset key is mapped", async () => assert.match(await read("app/app.js"), /resetImageZoom\(\{ announce: true \}\)/)],
  ["39 preview has four pan directions", async () => {
    ...
    assert.match(app, /function panImagePreview\(/);
  }],
  ["40 preview pan clamps per axis", async () => {
    const app = await read("app/app.js");
    assert.match(app, /function clampImagePreviewPan\(/);
    assert.match(app, /const limit = \(renderedSize - stageSize\) \/ 2/);
    assert.match(app, /offsetX: clampImagePreviewPan/);
    assert.match(app, /offsetY: clampImagePreviewPan/);
  }],
  ["41 preview reset clears pan", async () => assert.match(await read("app/app.js"), /state\.imagePanX = 0;\s+state\.imagePanY = 0;/)],
  ["42 preview uses existing status region", async () => {
    const app = await read("app/app.js");
    assert.match(app, /function announceImagePreviewZoom\(\)[\s\S]*?announceGalleryStatus/s);
    assert.match(app, /panImagePreview[\s\S]*?announceGalleryStatus/s);
  }],
  ["43 gallery busy has one helper", async () => assert.match(await read("app/app.js"), /function setGalleryBusy\(busy, requestId = null, request = null\)/)],
  ["44 request start sets busy true", async () => assert.match(await read("app/app.js"), /const requestId = \+\+assetRequestSequence;[\s\S]*?setGalleryBusy\(true, requestId, request\)/s)],
  ["45 success clears busy after render", async () => {
    const app = await read("app/app.js");
    const load = sliceBetween(app, "async function loadAssets(", "let libraryRefreshInFlight");
  ...
  ["46 error render clears busy", async () => {
    const app = await read("app/app.js");
    assert.match(app, /renderErrorState\(error, requestId, request\)/);
  ...
  ["47 stale request cannot clear busy", async () => assert.match(await read("app/app.js"), /if \(!busy && requestId !== null && !isCurrentAssetRequest\(requestId, request\)\) return false;/)],
  ["48 pagination uses same loader", async () => assert.match(await read("app/app.js"), /data-action="load-more".*?loadAssets\(\{ append: true \}\)/s)],
  ...
  ["55 nested dragenter only increments counter", async () => {
    const drag = functionSlice(await read("app/app.js"), "function setupDragDrop()", "// ===== Batch Operations =====");
  ...
  ["58 invalid drop keeps error Toast", async () => assert.match(await read("app/app.js"), /showToast\(t\("errorPathUnsupported"\), "error"\)/)],
  ["59 unavailable path keeps error Toast", async () => assert.match(await read("app/app.js"), /if \(!filePath\) \{[\s\S]*?showToast\(t\("dropPathUnavailable"\), "error"\)/)],
  ...
  ["62 F-08 empty state helper remains", async () => assert.match(await read("app/app.js"), /function deriveGalleryEmptyState\(\)/)],
  ["63 overlay manager remains shared", async () => assert.match(await read("app/app.js"), /const anchoredOverlayManager = createAnchoredOverlayManager\(\)/)],
  ["64 confirm dialog remains centralized", async () => assert.match(await read("app/app.js"), /function requestConfirmation\(/)],
  ["65 toast manager remains centralized", async () => assert.match(await read("app/app.js"), /function createToastManager\(/)],
  ["66 viewer sequence remains centralized", async () => assert.match(await read("app/app.js"), /assetViewSequence\.ids = state\.assets\.map/)],
  ...
  ["68 Cowart insert path remains local", async () => assert.match(await read("app/app.js"), /\/insert-cowart/)],
  ...
  ["72 implementation has no important override", async () => assert.doesNotMatch(await read("app/app.js"), /!important/)],
  ["73 preview zoom step is fixed", async () => assert.match(await read("app/app.js"), /const IMAGE_PREVIEW_ZOOM_STEP = 0\.25;/)],
  ["74 preview pan step is fixed", async () => assert.match(await read("app/app.js"), /const IMAGE_PREVIEW_PAN_STEP = 48;/)],
  ["75 preview transform preserves mouse input through Pointer Events", async () => {
    const app = await read("app/app.js");
    assert.match(app, /stage\.addEventListener\("pointerdown", handleImagePreviewPointerDown\)/);
    assert.match(app, /stage\.addEventListener\("pointermove", handleImagePreviewPointerMove\)/);
    assert.match(app, /stage\.addEventListener\("pointerup", handleImagePreviewPointerEnd\)/);
    assert.doesNotMatch(app, /document\.addEventListener\("mousemove"/);
    assert.doesNotMatch(app, /document\.addEventListener\("mouseup"/);
  }],
  ["76 wheel zoom suppresses announcement storm", async () => assert.match(await read("app/app.js"), /zoomImage\([^;]*\{ announce: false \}\)/)],
  ...
  ["80 drag overlay is viewer guarded", async () => assert.match(await read("app/app.js"), /if \(state\.viewMode !== "library"\) return;/)],
