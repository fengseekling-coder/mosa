...
test("8. sequence copies no asset objects", async () => {
  const app = await readApp();
  const slice = sliceBetween(app, "const assetViewSequence = {", "function openImportModal");
...
test("9. sequence source matches renderGrid result", async () => {
  ...
  assert.match(functionBody(app, "openAssetView"), /assetViewSequence\.ids = state\.assets\.map\(\(asset\) => asset\.id\);/, "sequence captures the same state.assets order");
});
...
test("10. search order captured", async () => {
  const app = await readApp();
  assert.match(functionBody(app, "buildAssetPageParams"), /project: request\.project, q: request\.query/, "request carries the search query");
  assert.match(functionBody(app, "loadAssets"), /requestAssetPage\(request/, "loadAssets delegates to the shared page request");
  assert.match(functionBody(app, "loadAssets"), /state\.assets = nextAssets;/, "state.assets holds the searched order");
});

// 11. The captured order carries the active facet filters.
test("11. filter order captured", async () => {
  const app = await readApp();
  assert.match(functionBody(app, "currentAssetRequest"), /facets: \{ \.\.\.state\.facets \}/, "request carries facets");
  assert.match(functionBody(app, "buildAssetPageParams"), /for \(const key of FACET_KEYS\)/, "facets are sent to the store");
});

// 12. The captured order carries the active sort.
test("12. sort order captured", async () => {
  const app = await readApp();
  assert.match(functionBody(app, "currentAssetRequest"), /sort: state\.sort/, "request carries the sort");
  assert.match(functionBody(app, "buildAssetPageParams"), /params\.set\("sort", request\.sort\)/, "sort is resolved by the store");
});
...
test("20. centralised navigate helper", async () => {
  const app = await readApp();
  assert.match(app, /function navigateAssetView\(direction\) \{/, "navigateAssetView exists");
...
test("27. navigation keeps return snapshot untouched", async () => {
  const app = await readApp();
  assert.doesNotMatch(stripJsComments(functionBody(app, "navigateAssetView")), /libraryReturnSnapshot/, "navigate helper does not reference the snapshot");
  assert.doesNotMatch(stripJsComments(functionBody(app, "updateAssetViewNav")), /libraryReturnSnapshot/, "boundary updates do not reference the snapshot");
});
...
test("29. return restores card focus", async () => {
  const app = await readApp();
  assert.match(functionBody(app, "returnToLibrary"), /cardButton\.focus\(\{ preventScroll: true \}\)/, "originating card focus kept");
});
...
test("37. async race guards exist", async () => {
  const app = await readApp();
  const render = functionBody(app, "renderAssetView");
  ...
  assert.match(functionBody(app, "handleAssetViewImageLoad"), /image\.dataset\.assetId !== state\.selectedId/, "load guard keyed to the current asset");
  assert.match(functionBody(app, "handleAssetViewImageError"), /image\.dataset\.assetId !== state\.selectedId/, "error guard keyed to the current asset");
});

// 38. A late load of a superseded image cannot overwrite the current asset.
test("38. stale load cannot overwrite", async () => {
  const app = await readApp();
  const load = stripJsComments(functionBody(app, "handleAssetViewImageLoad"));
...
test("39. stale error cannot overwrite", async () => {
  const app = await readApp();
  const error = stripJsComments(functionBody(app, "handleAssetViewImageError"));
...
test("41. navigation available on image error", async () => {
  ...
  assert.doesNotMatch(stripJsComments(functionBody(app, "updateAssetViewNav")), /assetViewImageReady|assetViewError/, "boundaries never gated on image state");
});
...
test("45. native disabled used", async () => {
  ...
  const app = await readApp();
  assert.match(functionBody(app, "setAssetViewControlDisabled"), /button\.disabled = disabled;/, "updates write the native disabled property");
});

// 46. aria-disabled stays in sync with native disabled.
test("46. aria-disabled in sync", async () => {
  const app = await readApp();
  const helper = functionBody(app, "setAssetViewControlDisabled");
  assert.match(helper, /button\.setAttribute\("aria-disabled", String\(disabled\)\);/, "aria-disabled mirrors native disabled");
  assert.match(functionBody(app, "updateAssetViewNav"), /setAssetViewControlDisabled\(els\.assetViewPrev/, "previous synced through the shared helper");
  assert.match(functionBody(app, "updateAssetViewNav"), /setAssetViewControlDisabled\(els\.assetViewNext/, "next synced through the shared helper");
});
...
test("49. phase 3B helpers intact", async () => {
  const app = await readApp();
  assert.match(app, /function computeAssetFitScale\(stageWidth, stageHeight, naturalWidth, naturalHeight\)/, "fitScale helper kept");
  assert.match(app, /function zoomAssetViewAtPoint\(/, "pointer-centred zoom helper kept");
  assert.match(app, /function clampAssetViewOffsets\(/, "clamp helper kept");
...
test("53. package manifest and lockfile unchanged", async () => {
  ...
  assert.equal(sha256(pkg), "e161974a477853703cc88724de39805fe5c65e590bd331060a17be6d087a2f24", "package.json must not change in Phase 3C");
