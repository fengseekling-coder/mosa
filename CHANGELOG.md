...
- Fixed the macOS Desktop/Web Capture KeepAlive startup race exposed by in-place app replacement. Desktop now publishes a short-lived PID/start-identity handoff marker before runtime startup; the supervisor stands down before spawning or gracefully yields an already-running child. Packaged Desktop also has a narrow verified-owner retirement fallback so an older already-loaded supervisor cannot reclaim port 43517 during the upgrade window. Stale markers and PID reuse fail open to normal background recovery.
