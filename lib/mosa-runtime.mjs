...
export async function startMosaRuntime(options = {}) {
  ...
  try {
    ...
    if (typeof store.cleanupOrphanedManagedFiles === "function") {
      await store.cleanupOrphanedManagedFiles().catch((error) => {
  ...
  try {
    ...
    const isWebCaptureRoute = isWebCapturePairRoute || isWebCaptureHealthRoute || url.pathname === "/api/ingest/web-capture" || url.pathname === "/api/web-capture";
