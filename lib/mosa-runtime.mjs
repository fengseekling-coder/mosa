...
export async function startMosaRuntime(options = {}) {
  ...
  try {
    ...
    const isWebCaptureRoute = isWebCapturePairRoute || url.pathname === "/api/ingest/web-capture" || url.pathname === "/api/web-capture";

    if (isWebCaptureRoute) {
      const originAllowed = isWebCapturePairRoute
    ...
    if (req.method === "OPTIONS") {
      const webCapturePreflightAllowed = isWebCapturePairRoute
        ? isApprovedExtensionOrigin(req.headers.origin, webCaptureOrigins)
        : isAllowedIngestOrigin(req.headers.origin, activePort, webCaptureOrigins);
