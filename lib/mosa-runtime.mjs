...
export async function startMosaRuntime(options = {}) {
  ...
  if (typeof store.purgeExpiredTrash === "function") {
    ...
    const purgeTrash = () => {
      ...
      purgeTrashInFlight = (async () => {
        await cleanupWebCaptureTemp(webCaptureIngest.tempRoot).catch((error) => {
          console.warn(`[MOSA] web-capture temp orphan sweep failed: ${error?.message || error}`);
        });
        ...
        do {
          const result = await store.purgeExpiredTrash({ cursor });
        ...
        if (removed && typeof webCaptureIngest?.pruneReferences === "function" && typeof store.listReferencedAttachmentIds === "function") {
  ...
  if (typeof store.cleanupOrphanedManagedFiles === "function") {
    // The derivative maintenance barrier already walks previews/mediums/thumbs.
    // Keep the age-bounded general sweep focused on originals in that case so
    // large libraries do not enumerate the same derivative trees twice at startup.
