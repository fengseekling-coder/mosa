...
export async function reconcileCodexGeneratedImages({ store, imagesDir, sessionsDir, projectId = DEFAULT_PROJECT_ID, knownHashes = null }) {
  ...
  for (const candidate of candidates) {
    ...
    if (knownPaths.has(candidate.imagePath)) {
      ...
      if (existingAsset && await upgradeToGenerationPrompt(store, existingAsset, generation)) updated.push(existingAsset.id);
...
async function upgradeToGenerationPrompt(store, asset, generation) {
  if (generation.promptStatus !== "image-generation-revised-prompt") return false;
  if (asset.prompt === generation.prompt && asset.source?.prompt_status === generation.promptStatus) return false;
...
function emptyTaskMetadata(taskId) {
  return {
    taskId,
    fallback: { taskId, prompt: "", promptStatus: "not-available", sessionPath: null, sessionUpdatedAt: null, callId: null, generatedAt: null },
