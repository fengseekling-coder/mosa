...
export async function reconcileCodexGeneratedImages({ store, imagesDir, sessionsDir, projectId = DEFAULT_PROJECT_ID, knownHashes = null }) {
  ...
  for (const asset of [...activeAssets, ...archivedAssets]) {
    for (const path of [asset.source?.path, asset.source?.codex_image_path].filter(Boolean)) {
      assetsBySourcePath.set(resolve(path), asset);
    }
  }
...
async function upgradeGenerationMetadata(store, asset, generation) {
  const hasGenerationPrompt = generation.promptStatus === "image-generation-revised-prompt";
  const promptChanged = hasGenerationPrompt
    && (asset.prompt !== generation.prompt || asset.source?.prompt_status !== generation.promptStatus);
  const modelChanged = Boolean(generation.model) && asset.source?.model !== generation.model;
  if (!promptChanged && !modelChanged) return false;
  await store.updateMetadata(asset.project_id, asset.id, {
    ...(promptChanged ? { prompt: generation.prompt, theme: promptTheme(generation.prompt) } : {}),
    business_fields: {
...
function metadataForCandidate(task, candidate) {
  return task.imagePrompts.get(candidate.imagePath) || task.fallback;
}

function promptTheme(prompt) {
  const match = /^Asset type:\s*(.+)$/mi.exec(String(prompt || ""));
  return match?.[1]?.trim() || "";
}
...
async function readTaskMetadataFile(taskId, sessionPath) {
  ...
  try {
    ...
    for (const line of raw.split("\n")) {
      ...
      const turnContext = event?.type === "turn_context" ? event.payload : null;
      if (turnContext) {
        currentModel = String(turnContext.model || "").trim() || currentModel;
        continue;
      }
      const generatedImage = event?.type === "event_msg" && event.payload?.type === "image_generation_end" ? event.payload : null;
      if (generatedImage) {
        const imagePath = typeof generatedImage.saved_path === "string" ? resolve(generatedImage.saved_path) : null;
        const prompt = String(generatedImage.revised_prompt || "").trim();
        if (imagePath && prompt) {
          imagePrompts.set(imagePath, {
            taskId,
            prompt,
            promptStatus: "image-generation-revised-prompt",
            sessionPath,
            sessionUpdatedAt: sessionStat.mtime.toISOString(),
            callId: String(generatedImage.call_id || "") || null,
            generatedAt: String(event.timestamp || "") || null,
            model: currentModel,
          });
        }
        continue;
...
function emptyTaskMetadata(taskId) {
  return {
    taskId,
    fallback: { taskId, prompt: "", promptStatus: "not-available", sessionPath: null, sessionUpdatedAt: null, callId: null, generatedAt: null, model: null },
