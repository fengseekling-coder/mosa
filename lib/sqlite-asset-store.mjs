...
function annotateAssetStackCovers(database, assets, projectId) {
  ...
  const rows = database.prepare(`
    WITH active_members AS (
      ...
      FROM asset_stack_members sm
      JOIN assets a ON a.project_id = sm.project_id AND a.id = sm.asset_id
      WHERE sm.project_id = ? AND a.deleted_at IS NULL AND a.archived = 0
    )
    SELECT asset_id, stack_id, member_count
    FROM active_members
    WHERE position = first_position AND asset_id IN (${placeholders})
  `).all(projectId, ...ids);
