  ...
  const store = {
    ...
    async deleteGroup(projectId, groupName, options = {}) {
      ...
      if (options.deleteAssets === "legacy-hard-delete") {
        ...
        invalidateFtsPageCache();
        // The database is now the authority. File cleanup is deliberately
        // best-effort and idempotent; any leftover managed files are harmless
        // orphans and will be swept by the existing repair/cleanup path.
        await Promise.all(deletionPlans.flatMap((plan) => plan.filesToUnlink).map((filePath) => unlink(filePath).catch(() => {})));
        return { success: true, name: storedName, deletedAssets: assignedRows.length };
      }
