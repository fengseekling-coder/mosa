...
export function createSqliteAssetStore(options = {}) {
  ...
  const store = {
    ...
    async updateGroupSettings(projectId, groupName, patch = {}) {
      ...
      const assignments = Object.entries(updates).map(([column, value]) => `${column} = @${column}`).join(", ");
      database.transaction(() => {
    ...
    async cleanupOrphanedDerivativeFiles({ olderThanMs = 0 } = {}) {
      const referenced = new Set();
      for (const row of database.prepare("SELECT preview_path, medium_path, thumbnail_path FROM assets").all()) {
