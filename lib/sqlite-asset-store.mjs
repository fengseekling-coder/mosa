  ...
  const store = {
    ...
    async findAutomaticIngestSuppression(projectId = DEFAULT_PROJECT_ID, hashes = {}) {
      ...
      const byContent = normalized.content_sha256
        ? database.prepare(`
          SELECT project_id, content_sha256, pixel_sha256, deleted_at, reason
      ...
      const byPixel = normalized.pixel_sha256
        ? database.prepare(`
          ...
          ORDER BY deleted_at DESC LIMIT 1
        `).get(cleanProjectId, normalized.pixel_sha256)
        : null;
    ...
    async listAutomaticIngestSuppressions(projectId = DEFAULT_PROJECT_ID) {
      ...
      return database.prepare(`
        ...
        WHERE project_id = ?
        ORDER BY deleted_at DESC, content_sha256, pixel_sha256
      ...
      if (cursor) {
        conditions.push(`(
          ...
          OR (deleted_at = @cursorDeletedAt AND (
            ...
            OR (content_sha256 = @cursorContentHash AND pixel_sha256 > @cursorPixelHash)
          ))
        )`);
    ...
    async clearAutomaticIngestSuppression(projectId = DEFAULT_PROJECT_ID, hashes = {}) {
      ...
      const result = database.prepare(`
        ...
        WHERE project_id = ?
          AND ((? != '' AND content_sha256 = ?) OR (? != '' AND pixel_sha256 = ?))
      `).run(
        cleanProjectId,
    ...
    async recordAutomaticIngestSuppression(projectId = DEFAULT_PROJECT_ID, record = {}) {
      ...
      if (!normalized.content_sha256 && !normalized.pixel_sha256) return null;
      database.prepare(`
        INSERT INTO automatic_ingest_suppressions (project_id, content_sha256, pixel_sha256, deleted_at, reason)
        VALUES (?, ?, ?, ?, ?)
        ON CONFLICT(project_id, content_sha256, pixel_sha256) DO UPDATE SET
    ...
    async createAsset(input = {}, context = {}) {
      ...
      const pixelHash = normalizeHash(input.source?.pixel_sha256);
      ...
      try {
        ...
        saveAsset(database, metadata, {
          ...
          rejectAutomaticIngestSuppression: context?.ingestMode === "automatic"
            ? { content_sha256: contentHash, pixel_sha256: pixelHash }
            : null,
        ...
        if (contentHash || pixelHash) {
          database.prepare(`
            ...
            ON CONFLICT(project_id, content_sha256, pixel_sha256) DO UPDATE SET
              deleted_at = excluded.deleted_at,
              reason = excluded.reason
          `).run(cleanProjectId, contentHash, pixelHash, now());
        }
      })();
      invalidateFtsPageCache();
...
function initializeSchema(database) {
  ...
  database.exec(`
    ...
    CREATE INDEX IF NOT EXISTS automatic_suppressions_project_deleted_idx ON automatic_ingest_suppressions(project_id, deleted_at DESC, content_sha256, pixel_sha256);
