...
import { createdAtTimestamp, normalizeCreatedAt, recentCutoffTimestamp } from "./recent-window.mjs";
import { buildRecipeSnapshot, normalizeSnapshotReferences } from "./recipe-snapshot.mjs";
import { assetSortName, normalizeAssetSort } from "./asset-sort.mjs";
import { relinkCodexAssets } from "./codex-hardlink.mjs";
...
export function createSqliteAssetStore(options = {}) {
  ...
  const store = {
    ...
    async listAssetPage(filters = {}) {
      ...
      const usesFts = Boolean(filters.query && useFts(filters.query));
      if (usesFts) {
