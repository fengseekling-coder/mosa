  ...
  const rows = database.prepare(`
    WITH requested_assets(asset_id) AS (
      ...
      SELECT sm.asset_id, sm.stack_id, sm.position,
        COUNT(*) OVER (PARTITION BY sm.project_id, sm.stack_id) AS member_count,
        MIN(sm.position) OVER (PARTITION BY sm.project_id, sm.stack_id) AS first_position
      FROM asset_stack_members sm
      JOIN assets a ON a.project_id = sm.project_id AND a.id = sm.asset_id
      WHERE sm.project_id = @projectId AND a.deleted_at IS NULL AND a.archived = 0
    )
    SELECT am.asset_id, am.stack_id, am.member_count, s.name
    FROM active_members am
    JOIN requested_assets requested ON requested.asset_id = am.asset_id
    JOIN asset_stacks s ON s.project_id = @projectId AND s.id = am.stack_id
    WHERE am.position = am.first_position
