...
function canonicalSourceSql(alias = "a") {
  const provider = `lower(COALESCE(json_extract(${alias}.source_json, '$.provider'), json_extract(${alias}.business_fields_json, '$.provider'), ''))`;
  const generationTool = `lower(COALESCE(json_extract(${alias}.source_json, '$.generation_tool'), json_extract(${alias}.business_fields_json, '$.generation_tool'), ''))`;
  return `CASE
    WHEN ${alias}.source_type IN ('web-chatgpt', 'web-gemini', 'web-flow', 'web-google-ai-studio', 'codex-generated', 'grok-generated', 'cowart-generated') THEN ${alias}.source_type
    WHEN ${provider} = 'chatgpt' THEN 'web-chatgpt'
    WHEN ${provider} = 'gemini' THEN 'web-gemini'
    WHEN ${provider} = 'flow' THEN 'web-flow'
    WHEN ${provider} = 'google-ai-studio' THEN 'web-google-ai-studio'
    WHEN ${generationTool} = 'codex' THEN 'codex-generated'
    WHEN ${generationTool} = 'grok' THEN 'grok-generated'
    WHEN ${generationTool} = 'cowart' THEN 'cowart-generated'
    ELSE ${alias}.source_type
  END`;
}
  ...
  const store = {
    ...
    async listGroups(projectId = DEFAULT_PROJECT_ID) {
      ...
      const sourceTypes = database.prepare(`
        SELECT ${canonicalSourceSql("a")} AS name, COUNT(*) AS count
        FROM assets a
...
function appendFilterConditions(conditions, params, filters) {
  if (filters.source) {
    conditions.push(`${canonicalSourceSql("a")} = @source`);
