...
export function createSqliteAssetStore(options = {}) {
  ...
  const store = {
    ...
    async purgeExpiredTrash({ nowMs = Date.now(), retentionMs = TRASH_RETENTION_MS } = {}) {
      ...
      const rows = database.prepare(`
        ...
        ORDER BY deleted_at, project_id, id LIMIT 500
      `).all(cutoff);
      let removed = 0;
      ...
      return { due: rows.length, removed, failed: pending.length, projects: [...removedProjects] };
    },
