...
export function createSqliteAssetStore(options = {}) {
  ...
  const store = {
    ...
    async listAssets(filters = {}) {
      const result = await this.listAssetPage({ ...filters, limit: 0, includeStackMembers: true });
    ...
    async listAssetStackAssets(projectId, stackId, filters = {}) {
      ...
      const searchTerms = normalizeAssetSearchQuery(filters.query).split(/\s+/u).filter(Boolean);
      searchTerms.forEach((term, index) => {
        const key = `stackLike${index}`;
        conditions.push(`LOWER(a.search_text) LIKE @${key} ESCAPE '\\'`);
    ...
    async createAssetStack(projectId, assetIds, options = {}) {
      ...
      database.transaction(() => {
        ...
        database.prepare("INSERT INTO asset_stacks (project_id, id, created_at, updated_at) VALUES (?, ?, ?, ?)")
          .run(cleanProjectId, stackId, timestamp, timestamp);
    ...
    async listAssetPage(filters = {}) {
      ...
      // A collapsed stack is one logical gallery node, but search still needs
      // to see every member behind that node. Only take the slightly richer
      // query path when this project actually has stacks; stack-free libraries
      // keep the existing FTS fast path unchanged.
      if (searchQuery && !filters.includeStackMembers && !filters.archived
        && database.prepare("SELECT 1 FROM asset_stacks WHERE project_id = ? LIMIT 1").get(projectId)) {
        return listStackAwareAssetPage(database, { filters, projectId, limit, sort, searchQuery, cursor });
      }
      const baseConditions = ["a.project_id = @projectId", filters.archived ? "a.archived = 1" : "a.archived = 0"];
      if (!filters.includeStackMembers) {
        baseConditions.push(`NOT EXISTS (
          SELECT 1 FROM asset_stack_members sm_hidden
          WHERE sm_hidden.project_id = a.project_id
            AND sm_hidden.asset_id = a.id
            AND sm_hidden.position > 0
        )`);
      }
    ...
    async archiveAsset(projectId, assetId) {
      ...
      return this.updateMetadata(cleanProjectId, cleanAssetId, { archived: true });
    },
...
function stackAwareMemberSearchSql(searchTerms, searchKind, params) {
  const conditions = [
    "m.project_id = a.project_id",
    "m.archived = 0",
    `(
      m.id = a.id
      OR m.id IN (
        SELECT sm_member.asset_id
        FROM asset_stack_members sm_cover
        JOIN asset_stack_members sm_member
          ON sm_member.project_id = sm_cover.project_id
         AND sm_member.stack_id = sm_cover.stack_id
        WHERE sm_cover.project_id = a.project_id
          AND sm_cover.asset_id = a.id
          AND sm_cover.position = 0
      )
    )`,
  ];
  const ftsTerms = searchTerms.filter((term) => [...term].length >= 3);
  const shortTerms = searchTerms.filter((term) => [...term].length < 3);
  if (ftsTerms.length) {
    params.stackNodeFtsQuery = buildFtsQuery(ftsTerms.join(" "));
    conditions.push("m.id IN (SELECT asset_id FROM asset_fts WHERE project_id = @projectId AND content MATCH @stackNodeFtsQuery)");
  }
  shortTerms.forEach((term, index) => {
    const key = `stackNodeLike${index}`;
    conditions.push(`LOWER(m.search_text) LIKE @${key} ESCAPE '\\'`);
    params[key] = `%${escapeLikePattern(term)}%`;
  });
  // One/two-character-only queries never touch FTS, so every term must still
  // participate in the AND membership test.
  if (!ftsTerms.length && !shortTerms.length) {
    searchTerms.forEach((term, index) => {
      const key = `stackNodeLike${index}`;
      conditions.push(`LOWER(m.search_text) LIKE @${key} ESCAPE '\\'`);
      params[key] = `%${escapeLikePattern(term)}%`;
  ...
  if (searchKind) {
    params.stackSearchKind = searchKind;
    conditions.push(`mosa_asset_matches_search_kind(
      @stackSearchKind, m.asset, m.id, m.prompt, m.skill, m.style, m.theme, m.group_name, m.category, m.tags_text
    ) = 1`);
  }
  return conditions.join(" AND ");
}

function listStackAwareAssetPage(database, { filters, projectId, limit, sort, searchQuery, cursor }) {
  const searchTerms = searchQuery.split(/\s+/u).filter(Boolean);
  const searchKind = searchTerms.length ? assetSearchKind(searchQuery) : "";
  const params = { projectId };
  ...
  appendFilterConditions(baseConditions, params, filters);
  const memberSearchSql = stackAwareMemberSearchSql(searchTerms, searchKind, params);
  const nodeMatchSql = `EXISTS (SELECT 1 FROM assets m WHERE ${memberSearchSql})`;
  const pageConditions = [...baseConditions, nodeMatchSql];
  const orderBy = (alias = "a") => sort === "name"
    ? `${alias}.sort_name ASC, ${alias}.id ASC`
    : (sort === "oldest" ? `${alias}.created_at ASC, ${alias}.id ASC` : `${alias}.created_at DESC, ${alias}.id DESC`);
  ...
  let select;
  if (searchKind) {
    const memberScoreSql = buildSearchScoreSql(searchTerms, params, "m");
    const nodeScoreSql = `(SELECT MAX(${memberScoreSql}) FROM assets m WHERE ${memberSearchSql})`;
    select = `WITH ranked AS MATERIALIZED (
      SELECT ${PAGE_ASSET_COLUMNS}, ${nodeScoreSql} AS _search_score
      FROM assets a
      WHERE ${pageConditions.join(" AND ")}
    )
    SELECT r.* FROM ranked r
    ...
    select = `SELECT ${PAGE_ASSET_COLUMNS}, 1 AS _search_score
      FROM assets a
      WHERE ${pageConditions.join(" AND ")}
      ORDER BY ${orderBy("a")}`;
  }
  ...
  const total = Number(database.prepare(`
    ...
    WHERE ${baseConditions.join(" AND ")} AND ${nodeMatchSql}
  `).get(params).count || 0);
  const last = pageRows.at(-1);
  return {
    assets,
