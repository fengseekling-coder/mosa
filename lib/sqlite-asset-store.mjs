  ...
  const store = {
    ...
    async createAsset(input = {}, context = {}) {
      ...
      try {
        ...
        saveAsset(database, metadata, { enqueueDerivative, insertOnly: true });
      } catch (error) {
...
async function resolveReadableImagePath(store, value, trustedSourceRoots = []) {
  ...
  if (!MEDIA_EXTENSIONS.has(extname(requestedPath).toLowerCase())) throw new Error(`Unsupported media type: ${requestedPath}`);
