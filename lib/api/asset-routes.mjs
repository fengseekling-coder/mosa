...
export async function handleAssetRoute({ req, res, url, context }) {
  ...
  if (stackAssetsMatch && req.method === "GET") {
    ...
    sendJson(res, 200, await store.listAssetStackAssets(url.searchParams.get("project") || "default", stackId, filters));
    return true;
  }
  ...
  if (assetMatch && req.method === "DELETE") {
    ...
    if (typeof webCaptureIngest?.pruneReferences === "function") {
      try {
        const referencedIds = await referencedAttachmentIds(store, projectId);
