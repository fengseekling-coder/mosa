...
import { MOSA_RESERVED_PRODUCTION_PORTS } from "./runtime-defaults.mjs";
import { isPathInside, pathsEqual } from "./path-safety.mjs";
 ...
 *     Defaults to the desktop discovery ports plus legacy production ports.
...
export function validateRuntimeIsolation(params = {}) {
  ...
  // For a path that exists, use realpath. For a dangling symlink, follow its
  // link text before walking an existing ancestor, so QA cannot use a broken
  // alias to disguise a protected production path on a fresh machine.
  function safeCanonical(path, seenLinks = new Set()) {
    const resolved = resolve(path);
    const tail = [];
    let current = resolved;
    while (true) {
      try {
        return resolve(realpathSync(current), ...tail.slice().reverse());
      } catch {}
      try {
        const target = readlinkSync(current);
        if (!seenLinks.has(current)) {
          seenLinks.add(current);
          return safeCanonical(resolve(dirname(current), target, ...tail.slice().reverse()), seenLinks);
        }
      } catch {}
      ...
      // Preserve the missing tail while continuing toward an existing parent.
  ...
  if (pathsEqual(canonLib, prodLib)) {
    return {
      ok: false,
  ...
  if (isPathInside(prodLib, canonLib)) {
    return {
      ok: false,
  ...
  if (!pathsEqual(canonActual, canonExpected)) {
    return {
      ok: false,
  ...
  if (defaultUserData) {
    ...
    if (pathsEqual(canonActual, canonDefault)) {
      return {
        ok: false,
    ...
    if (isPathInside(canonDefault, canonActual)) {
      return {
        ok: false,
