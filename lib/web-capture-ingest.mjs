...
async function inspectImageBytes(imageBytes) {
  try {
    ...
    // sharp reports a single frame's height for animations and stacks the rest
    // into `pages`, so budgeting on width * height alone lets a multi-frame GIF
    // or AVIF past a limit its decoded pixels blow through.
    ...
    return { format: String(metadata.format || ""), width, height: frameHeight, pixelHash: await pixelDigest(imageBytes) };
  } catch (error) {
