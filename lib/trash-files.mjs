...
async function readJournal(projectDir, stageDir) {
  ...
  return parsed.entries.map((entry) => {
    const source = resolve(String(entry?.source || ""));
    const target = resolve(String(entry?.target || ""));
    if (!isPathInside(projectDir, source) || !isPathInside(stageDir, target)) {
      throw new Error("Permanent-deletion recovery journal contains a path outside its project boundary.");
    }
...
export async function stageFilesForPermanentDeletion(projectDir, filePaths = []) {
  ...
  const outsideRoot = uniquePaths.find((filePath) => !isPathInside(root, filePath));
  if (outsideRoot) {
    throw new Error(`Refusing to permanently delete a file outside the project directory: ${outsideRoot}`);
  }
  const realRoot = await realpath(root).catch(() => root);
  for (const source of uniquePaths) {
    const realSource = await realpath(source).catch((error) => {
      if (error?.code === "ENOENT") return null;
      throw error;
    });
    if (realSource && !isPathInside(realRoot, realSource)) {
      throw new Error(`Refusing to permanently delete a symlink-resolved file outside the project directory: ${source}`);
    }
  }
...
export async function cleanupPermanentDeletionStaging(projectDir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.name.startsWith(STAGED_PURGE_PREFIX)) {
      try {
        const journal = await readJournal(root, path);
