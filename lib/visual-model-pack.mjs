...
async function verifyVisualRuntimeDescriptor({ runtime, files, packDir }) {
  ...
  const onnxEntry = find("onnxruntime-node/dist/index.js");
  const commonEntry = find("onnxruntime-common/dist/cjs/index.js");
  const tokenizerEntry = find("@huggingface/tokenizers/dist/tokenizers.cjs");
  if (!onnxPackage || !commonPackage || !tokenizerPackage || !onnxEntry || !commonEntry || !tokenizerEntry) {
