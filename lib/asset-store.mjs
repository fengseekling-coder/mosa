...
export function createJsonAssetStore(options = {}) {
  ...
  return {
    ...
    async listAssetPage(filters = {}) {
      ...
      let start = 0;
      if (filters.cursor) {
        const cursor = parseAssetCursor(filters.cursor);
        start = assets.findIndex((asset) => String(asset.created_at || "") < cursor.createdAt
          || (String(asset.created_at || "") === cursor.createdAt && String(asset.id || "") < cursor.id));
...
function parseAssetCursor(value) {
  try {
...
async function resolveReadableImagePath(store, value, trustedSourceRoots = []) {
  ...
  if (!MEDIA_EXTENSIONS.has(extname(requestedPath).toLowerCase())) {
    throw new Error(`Unsupported media type: ${requestedPath}`);
  }
