  ...
  return {
    ...
    async deleteGroup(projectId, groupName, options = {}) {
      ...
      return withGroupWriteLock(this, cleanProjectId, async () => {
        ...
        if (options.deleteAssets === true && groupAssets.length) {
          ...
          await withSuppressionWriteLock(this, cleanProjectId, async () => {
            ...
            try {
              ...
              const nextNames = names.filter((_, itemIndex) => itemIndex !== index);
              await writeGroupNames(this, cleanProjectId, nextNames);
            } catch (error) {
              await Promise.allSettled(changed.map((asset) => writeMetadata(this, asset)));
              await writeGroupNames(this, cleanProjectId, names).catch(() => {});
              await writeAutomaticIngestSuppressions(this, cleanProjectId, previousSuppressions).catch(() => {});
              throw error;
            }
          });
          return { success: true, name, deletedAssets: activeAssets.length, trashed: true };
        } else {
        ...
        return { success: true, name, deletedAssets: options.deleteAssets === true ? groupAssets.length : 0 };
      });
    },
    /**
    ...
    async restoreAsset(projectId, assetId) {
      ...
      if (restored.group) await ensureGroup(this, cleanProjectId, restored.group);
...
async function prepareJsonAssetDeletion(store, projectId, asset) {
  const metadataPath = join(store.metadataDir(projectId), `${asset.id}.json`);
  ...
  const pixelHashVersion = recomputedPixelHash
    ? PIXEL_HASH_VERSION
    : (pixelHash ? storedPixelHashVersion : "");
  return { asset, metadataPath, managedPaths, contentHash, pixelHash, pixelHashVersion };
}

function mergeDeletionSuppression(records, projectId, plan, deletedAt) {
  if (!plan.contentHash && !plan.pixelHash) return records;
  const normalized = normalizeSuppressionRecord({
    project_id: projectId,
    content_sha256: plan.contentHash,
    pixel_sha256: plan.pixelHash,
  ...
  const index = records.findIndex((item) => item.content_sha256 === normalized.content_sha256
    && item.pixel_sha256 === normalized.pixel_sha256
    && String(item.pixel_hash_version || "") === String(normalized.pixel_hash_version || ""));
  if (index === -1) return [...records, normalized];
  return records.map((item, itemIndex) => itemIndex === index ? normalized : item);
}

async function deleteJsonGroupAssetsAtomically(store, projectId, groupAssets, nextGroupNames) {
  // Complete every path/hash/read preflight before touching persisted state.
  // The JSON store cannot offer SQLite's single-file transaction, so metadata
  // is first renamed into a same-volume staging directory and all small JSON
  // indexes are snapshotted for rollback. Media bytes are deleted only after
  // the logical commit succeeds.
  const plans = [];
  for (const asset of groupAssets) plans.push(await prepareJsonAssetDeletion(store, projectId, asset));

  return withSuppressionWriteLock(store, projectId, async () => withGenerationHistoryLock(store, projectId, async () => {
    const previousGroups = await readGroupNames(store, projectId);
    const previousSuppressions = await readAutomaticIngestSuppressions(store, projectId);
    const previousEvents = await readJsonArray(store.generationEventsFile(projectId));
    const previousRelations = await readJsonArray(store.generationRelationsFile(projectId));
    const previousCandidates = await readJsonArray(store.generationRelationCandidatesFile(projectId));
    const removedAssetIds = new Set(plans.map((plan) => plan.asset.id));
    const removedEventIds = new Set(previousEvents
      .filter((event) => removedAssetIds.has(String(event?.output_asset_id || "")))
      .map((event) => String(event?.id || ""))
      .filter(Boolean));
    const nextEvents = previousEvents.filter((event) => !removedEventIds.has(String(event?.id || "")));
    const nextRelations = previousRelations.filter((relation) => (
    ...
    const deletedAt = new Date().toISOString();
    const nextSuppressions = plans.reduce(
      (records, plan) => mergeDeletionSuppression(records, projectId, plan, deletedAt),
      previousSuppressions,
    );
    const stagingDir = join(store.metadataDir(projectId), `.delete-group-${randomUUID()}`);
    await mkdir(stagingDir, { recursive: false });
    const staged = [];
    let logicalStateChanged = false;
    try {
      for (const plan of plans) {
        const stagedPath = join(stagingDir, `${plan.asset.id}.json`);
        await rename(plan.metadataPath, stagedPath);
        staged.push({ ...plan, stagedPath });
      }

      await writeAutomaticIngestSuppressions(store, projectId, nextSuppressions);
      await Promise.all([
        atomicWriteFile(store.generationEventsFile(projectId), `${JSON.stringify(nextEvents, null, 2)}\n`),
        atomicWriteFile(store.generationRelationsFile(projectId), `${JSON.stringify(nextRelations, null, 2)}\n`),
        atomicWriteFile(store.generationRelationCandidatesFile(projectId), `${JSON.stringify(nextCandidates, null, 2)}\n`),
      ]);
      await writeGroupNames(store, projectId, nextGroupNames);
      logicalStateChanged = true;
    } catch (error) {
      // Restore all small indexes first, then put metadata files back in place.
      // allSettled is intentional here: rollback makes a best effort across
      // independent files instead of abandoning the rest after one failure.
      await Promise.allSettled([
        writeGroupNames(store, projectId, previousGroups),
        writeAutomaticIngestSuppressions(store, projectId, previousSuppressions),
        atomicWriteFile(store.generationEventsFile(projectId), `${JSON.stringify(previousEvents, null, 2)}\n`),
        atomicWriteFile(store.generationRelationsFile(projectId), `${JSON.stringify(previousRelations, null, 2)}\n`),
        atomicWriteFile(store.generationRelationCandidatesFile(projectId), `${JSON.stringify(previousCandidates, null, 2)}\n`),
      ]);
      for (const plan of [...staged].reverse()) {
        await rename(plan.stagedPath, plan.metadataPath).catch(() => {});
      }
      await rmdir(stagingDir).catch(() => {});
      throw error;
    }

    if (!logicalStateChanged) throw new Error("Group deletion did not reach a logical commit.");
    await Promise.allSettled(staged.map((plan) => unlink(plan.stagedPath)));
    await rmdir(stagingDir).catch(() => {});
    await Promise.allSettled(plans.flatMap((plan) => plan.managedPaths).map((filePath) => unlinkIfPresent(filePath)));
    return plans.length;
  }));
}
