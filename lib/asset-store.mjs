    ...
    async deleteGroup(projectId, groupName, options = {}) {
      ...
      return withGroupWriteLock(this, cleanProjectId, async () => {
        ...
        if (options.deleteAssets === true && groupAssets.length) {
          const deletingIds = new Set(groupAssets.map((asset) => asset.id));
          const externalChild = assets.find((asset) => asset.parent_asset_id && deletingIds.has(asset.parent_asset_id) && !deletingIds.has(asset.id));
          if (externalChild) {
            throw assetStoreError(
              "VERSION_PARENT_HAS_CHILDREN",
              `Cannot delete group ${name}; version parent ${externalChild.parent_asset_id} has child ${externalChild.id} outside the group.`,
            );
          }
          const nextNames = names.filter((_, itemIndex) => itemIndex !== index);
          await deleteJsonGroupAssetsAtomically(this, cleanProjectId, groupAssets, nextNames);
          return { success: true, name, deletedAssets: groupAssets.length };
        } else {
