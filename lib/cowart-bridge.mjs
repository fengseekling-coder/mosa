...
/**
 * Reconciles one Cowart canvas into a MOSA library. A bridge manager can run
 * several instances, but each instance remains explicitly project-scoped.
 */
export function createCowartAssetBridge(options = {}) {
  ...
  if (!store || typeof store.createAsset !== "function" || typeof store.listAssets !== "function") {
    throw new Error("Cowart bridge requires a MOSA store.");
  }
...
export async function reconcileCowartAssets({ store, canvasDir, projectId = DEFAULT_PROJECT_ID, cowartProjectDir = null, sourceId = null }) {
  ...
  for (const candidate of candidates) {
    if (candidate.mosaAssetId) {
      skipped.push({ path: candidate.imagePath, reason: "mosa-origin" });
      continue;
    }
...
async function readCowartAssetCandidates(canvasDir) {
  ...
  for (const pageEntry of pageEntries) {
    ...
    for (const record of Object.values(records)) {
      ...
      const mosaAssetId = assetMeta.mosaAssetId
        || assetMeta.mosa_asset_id
        || shapeMeta.mosaAssetId
        || shapeMeta.mosa_asset_id
        || null;
      candidates.push({
        canvasDir: root,
