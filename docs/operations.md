...
Packaged macOS arm64 builds use the same Settings update control. The trusted main process re-reads `releases/latest.json` and accepts only `platforms.macos` entries whose identity is exactly `macOS` / `arm64` / `MOSA-darwin-arm64-<version>.zip`. It downloads only from `https://mosa.azhuilab.com/downloads/`, rejects redirects, verifies declared size and SHA-256, and stages the ZIP under Electron `userData`. A detached `/bin/sh` helper waits for the old Desktop process to exit, expands the ZIP with `/usr/bin/ditto`, verifies the replacement bundle id/version and `codesign --verify --deep --strict`, parks the old `/Applications/MOSA.app` as a transaction backup, installs the replacement, and launches it with a one-time readiness marker. The updated app writes that marker only after its renderer and local runtime are ready. Missing readiness, an early crash, or any replacement failure restores the previous app and relaunches it automatically. This path deliberately reuses the Desktop/Web Capture runtime handoff protocol before the old process exits so the KeepAlive supervisor cannot steal port `43517` during the replacement window.
